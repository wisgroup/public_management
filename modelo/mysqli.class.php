...
Class WISMysqli {
     ...
     */
    public function query($query, $print = false) {

        if ($print) {
            echo '<div class="wis-debugx">'.$query . " </div>"; //exit;
        }
        $result = mysqli_query($this->link, $query) or die(mysqli_error($this->link) . ' SQL:' . $query);
        $this->result = $result;
     ...
     */
    public function update(array $values, $table, $where = false, $limit = false, $print = false) {
        ...
        if ($this->query("UPDATE `" . $table . "` SET " . implode($fields, ", ") . $where . $limit, $print)) {
            return true;
        } else {
            return false;
        }
    ...
    public function update_number(array $values, $table, $where = false, $limit = false, $print = false) {
     ...
     * @return boolean Result
     */
    public function insert(array $values, $table, $print = false) {
        if (count($values) < 0) {
            return false;
        }
        ...
        //echo "INSERT INTO `" . $table . "`(`" . implode(array_keys($values), "`, `") . "`) VALUES (" . implode($values, ", ") . ")";
        if ($this->query("INSERT INTO `" . $table . "`(`" . implode(array_keys($values), "`, `") . "`) VALUES (" . implode($values, ", ") . ")", $print)) {
            return true;
        } else {
            return false;
        }
     ...
     */
    public function select($fields, $table, $where = false, $orderby = false, $limit = false, $print = false) {
        if (is_array($fields)) {
        ...
        // echo "<br>Where: ";print_r($where);
        /* $query = "<pre> SELECT " . $fields . " FROM " . $table . "" . $where . $orderby . $limit; */

        if ($table === 'opcion p') {
            //echo $query;
        }
        /* echo "SELECT " . $fields . " FROM " . $table . "" . $where . $orderby . $limit; */
        $this->query("SELECT " . $fields . " FROM " . $table . "" . $where . $orderby . $limit, $print);
     ...
     */
    public function selectOne($fields, $table, $where = false, $orderby = false, $print = false) {
        $result = $this->select($fields, $table, $where, $orderby, '1', $print);
        return $result[0];
    }
    ...
    /**
     * Select Distinct 
     * @atuor Sebastian Florez 2014-10-15
     *
     * @param mixed $fields Array or string of fields to retrieve
     * @param string $table Table to retrieve from
     * @param string $where Where condition
     ...
     * @return array Array of rows
     */
    public function selectDistinct($fields, $table, $where = false, $orderby = false, $limit = false) {
        if (is_array($fields)) {
            $fields = "" . implode($fields, ",") . "";
        }
        ...
        $this->query("SELECT DISTINCT " . $fields . " FROM " . $table . "" . $where . $orderby . $limit);
     ...
     * @return boolean Result
     */
    public function updateOperacion(array $values, $table, $where = false, $limit = false, $operacion = "") {
        if (count($values) < 0)
            return false;
        ...
        foreach ($values as $field => $val) {
            if (($field == 'pesos' || $field == 'comision' || $field == 'cartera' || $field == 'mensajes') && ( $table == 'saldo' || $table == 'proveedor_saldo') && $operacion != "") {
                $fields[] = "`" . $field . "` = " . $field . $operacion . $this->escapeString($val) . "";
            } else {
                $fields[] = "`" . $field . "` = '" . $this->escapeString($val) . "'";
            }
        }

        // echo "<br>campos :";print_r(implode($fields, ", "));
     ...
     * @return boolean Result
     */
    public function updateOperacionQuery(array $values, $table, $where = false, $limit = false, $operacion = "") {
        ...
        foreach ($values as $field => $val) {
            $operacion_suma = strpos($field, '+');
            $operacion_resta = strpos($field, '-');
            $operacion = '';

            if ($operacion_suma !== false) {
                $operacion = '+';
                $field = str_replace('+', '', $field);
            } else if ($operacion_resta !== false) {
                $operacion = '-';
                $field = str_replace('-', '', $field);
            }
