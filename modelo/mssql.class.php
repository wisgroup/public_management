...
Class WISMsSql {
    ...
    public function connect($persist = true) {
        if ($persist) {
			$link = odbc_pconnect("Driver={SQL Server Native Client 10.0};Server=$this->host;Database=$this->db_name;", $this->user, $this->pass);
		}else {
            $link = odbc_connect("Driver={SQL Server Native Client 10.0};Server=$this->host;Database=$this->db_name;", $this->user, $this->pass);
		}
        if (!$link) {
    ...
    public function insert(array $values, $table) {
        ...
        if ($this->query("INSERT INTO `" . $table . "`(`" . implode(array_keys($values), "`, `") . "`) VALUES (" . implode($values, ", ") . ")", false)) {
            return true;
        } else {
            return false;
		}
