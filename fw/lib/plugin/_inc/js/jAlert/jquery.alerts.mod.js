...
(function($) {
	...
	$.alerts = {
		...
		load: function(message, title, callback) {
			...
			if( message == null ) message = 'Por favor espere. . .';
		...
		_show: function(title, msg, value, type, callback) {
			...
			// IE6 Fix
			// var pos = (parseInt($.browser.version) <= 6 ) ? 'absolute' : 'fixed'; 
			...
			switch( type ) {
				...
				case 'alert':
					$("#popup_message").after('<div id="popup_panel"><input onfocus="this.blur()" type="button" value="' + $.alerts.okButton + '" id="popup_ok" /></div>');
				...
				case 'confirm':
					$("#popup_message").after('<div id="popup_panel"><input onfocus="this.blur()" type="button" value="' + $.alerts.okButton + '" id="popup_ok" /> <input onfocus="this.blur()" type="button" value="' + $.alerts.cancelButton + '" id="popup_cancel" /></div>');
					$("#popup_ok").click( function() {
						$.alerts._hide();
				...
				case 'prompt':
					$("#popup_message").append('<br /><input type="text" size="30" id="popup_prompt" />').after('<div id="popup_panel"><input onfocus="this.blur()" type="button" value="' + $.alerts.okButton + '" id="popup_ok" /> <input onfocus="this.blur()" type="button" value="' + $.alerts.cancelButton + '" id="popup_cancel" /></div>');
		...
		_reposition: function() {
			...
			// IE6 fix
			// if(parseInt($.browser.version) <= 6 ) top = top + $(window).scrollTop();
	...
	// Funciones para ventana Loading 
	// Autor: Sebastian Florez
	// Fecha: 29-04-2015

	jClose = function(message, title, callback) {
		$.alerts.close(message, title, callback);
	}
